...
export const highlight = (d, cases) => {
  ...
  d3.selectAll(".nodes")
    .attr("r", (d) => d.r)
    .style("opacity", 0.3);
  d3.selectAll(".links").style("stroke", "#999").style("opacity", 0.3);
  d3.selectAll(".node-labels > text").style("opacity", 0.3);
  ...
  d3.select("#CO-" + caseId)
    .attr("r", (d) => 2 * d.r)
    .style("opacity", 1);
  d3.selectAll(".CO-labels-self-" + caseId).style("opacity", "1");
  d3.selectAll(".CO-links-" + caseId)
    ...
    .on("end", (d, i) => {
      if (i === 0) {
        d3.selectAll(".CO-nodes-" + caseId).style("opacity", "1");
        d3.selectAll(".CO-labels-" + caseId).style("opacity", "1");
      }
    });
...
export const tooltipHTML = (d) => {
  if (d.properties !== undefined) {
    ...
    let cazuriInfo = d.infected_persons
        ? d.infected_persons + " " + labels.valueLabel[language] + ".<br />"
        : "",
      genderInfo =
      ...
      ageInfo =
        d.properties.age != null && d.properties.age != 0
          ? ", " + d.properties.age
          : "",
      countyInfo =
        d.properties.county != null && d.properties.county != ""
          ? ", " + d.properties.county
          : "",
      statusInfo =
        d.properties.status != null
          ...
          : "",
      diagnosticDateInfo =
        d.properties.diagnostic_date !== null
          ? labels.confdateLabel[language] +
            ": " +
            d.properties.diagnostic_date +
            ".<br />"
          : "",
      healingDateInfo =
        d.properties.healing_date !== null
          ? labels.recoverydateLabel[language] +
            ": " +
            d.properties.healing_date +
            ".<br />"
          : "",
      countyOfInfectionInfo =
        ...
        d.properties.country_of_infection !== "Romania"
          ? labels.infectionCountryLabel[language] +
            ": " +
            d.properties.country_of_infection +
            ".<br />"
          : "",
      referenceInfo =
        d.properties.reference !== null && d.properties.reference !== ""
...
export const unHighlight = () => {
  ...
  d3.selectAll(".node-labels > text").style("opacity", "1");
};

export const hideTooltip = () => {
  d3.selectAll(".nodes").attr("r", (d) => d.r);
  tooltip_div.transition().duration(200).style("opacity", 0);
};

export const highlightSearchedId = (caseId) => {
  d3.select("#CO-" + caseId).dispatch("mouseover");
  // .dispatch('click');
};
...
export const toggleInfo = (infoStatus, language) => {
  if (infoStatus === true) {
    tooltip_div
      ...
      .style("opacity", 0.9);
    tooltip_div
      .html(Language.infoHtml(language))
      .style("left", "50px")
      .style("top", "50px")
